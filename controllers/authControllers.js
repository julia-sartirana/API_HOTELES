...
exports.protectRoute = async (req, res, next) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET, async (err, decaded) => {
    if (err) {
      res.status(401).json({
        status: 'fail',
    ...
    let user = await User.findOne({_id: decaded.id}).select('+role')
    req.user = user;
...
exports.onlyAdmin = (req, res, next) => {
 if(!req.user.role || req.user.role !== 'admin') {
   res.status(401).json({
     status: 'fail',
     message: 'No estas autorizado a realizar esta acción'
    })
  }
  next();
}